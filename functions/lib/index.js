...
exports.scanElectricityBill = exports.getBCXCopilotReply = exports.getAIRecommendations = void 0;
...
exports.getAIRecommendations = functions.https.onCall(async (data) => {
    if (!genAI) {
...
exports.getBCXCopilotReply = functions.https.onCall(async (data) => {
    if (!genAI) {
        throw new functions.https.HttpsError('failed-precondition', 'Gemini API not configured');
    }
...
CRITICAL RULE: Only answer questions related to the BCX platform, carbon tracking, climate change, GHG protocols, sustainability workflows, or the features above. Keep answers concise and helpful.`;
    try {
        ...
        const history = Array.isArray(data.history) ? data.history.map((msg) => ({
...
exports.scanElectricityBill = functions.https.onCall(async (data) => {
    if (!genAI) {
        throw new functions.https.HttpsError('failed-precondition', 'Gemini API not configured');
    }
    const imagePart = {
